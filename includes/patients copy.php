...
function prescribe($report_id, $medicine, $doses, $days, $reply, $notes, $medicine_charge, $price_set)
{
	global $con;
	...
	if ($days == 0) {
		...
		if ($medicine_charge == 'yes') {
			$total_charge = $medicine_price_coba * $total;
		...
		$notes .= "$medicine - $doses  for $days days<br>";
	}
	mysqli_query($con, "update p_reports set
