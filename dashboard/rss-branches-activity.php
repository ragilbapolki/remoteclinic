...
function drawChart() {
    var data = google.visualization.arrayToDataTable([
        ...
        <?php $branch_id = staff_info('branch');
      $get_diff = get_global('recent_hours') / 24;
      for ($k = 0; $k < get_global('recent_hours'); $k++) {
        $k = $k + $get_diff; ?>[
            '', <?php echo count_patients($branch_id, $k) ?>, <?php echo count_reports($branch_id, $k) ?>,
            <?php echo count_lstaff($branch_id, $k) ?>],
        <?php } ?>
    ]);
...
<div id="curve_chart" style="width: 100%; height: 165px;" class="right-chart"></div>
