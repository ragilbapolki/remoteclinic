                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            <h4 class="card-title">Recent Activity List</h4>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            function drawChart() {
                                var data = google.visualization.arrayToDataTable([
                                    ...
                                    for ($k = 0; $k < get_global('recent_hours'); $k++) {
                                        $k = $k + $get_diff; ?>[
                                            '-', <?php echo count_patients($show_branch, $k) ?>,
                                            <?php echo count_reports($show_branch, $k) ?>],
                                    <?php } ?>
                                ]);
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td class="symptoms"><a href="../patients/reports<?php echo $extension; ?>?id=<?php echo $reports['id'] ?>"><?php echo substr($reports['symptoms'], 0, 80); ?>...</a>
                                        </td>
                                        <td class="branch-name">
                                            <?php if ($reports['branch'] != staff_info('branch')) echo "[" . branch_info('name', $reports['branch']) . "]"; ?>
                                        </td>
                                        <td class="status"><?php if ($reports['signed_by'] != "") { ?><span class="s"><?php echo staff_info("full_name", $reports['signed_by']); ?></span><?php } ?><?php if ($reports['signed_by'] == "" && $reports['engaged_by'] != "") { ?><span class="e">(Assigned to
                                                    <?php echo staff_info("full_name", $reports['engaged_by']); ?>)</span><?php } ?><?php if ($reports['signed_by'] == "" && $reports['engaged_by'] == "") { ?><span class="p">(Pending)</span><?php } ?></td>
